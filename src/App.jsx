...
import { materializeAiFlashcard } from "./lib/aiProvenance.js";
...
export default function App() {
  ...
  const updateSettings = useCallback((patch) => {
    setProfile((current) => ({ ...current, settings: { ...current.settings, ...patch } }));
  }, []);
    ...
    <div className="app-shell">
      ...
      <div className="app-main">
        ...
        <nav className="bottom-nav" aria-label="Mobile navigation">
          {BOTTOM_VIEW_ITEMS.map(({ id, label, icon: Icon }) => <button className={view === id ? "active" : ""} onClick={() => changeView(id)} key={id} type="button"><Icon size={20} /><span>{label}</span></button>)}
        </nav>
      </div>
