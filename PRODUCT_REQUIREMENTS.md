...
|---|---|---|---|
| BUG-001 | Reliable mobile top-menu, sidebar close, scrim close, Escape close, and navigation close behavior | `Implemented` | Main paths pass `audit:workflow`; focus-return, scroll-lock, and sidebar-action assertions remain |
| BUG-002 | Reliable straight-line whiteboard tool for touch, Apple Pencil, and mouse | `Partial` | Touch line and stored two-point object covered; pen/mouse-line, tap rejection, full transform/export, and restore coverage remain |
| BUG-003 | Systematic interaction audit for every visible control and disabled state | `Partial` | Static accessible-name/touch-size audit exists; action contracts, disabled reasons, and every dialog focus cycle remain |
...
| LEARN-001 | Durable text-anchored highlights and annotations | `Partial` | Core creation, paint, edit, filtering, card conversion, persistence, and reload covered; relocation/orphan/export/restore coverage remains |
| LEARN-002 | Convert a highlight, clipping, concept, or custom prompt into a review card | `Partial` | Eight card types, duplicate prevention, safe Markdown preview, edit/archive/restore, and blank/clipping/highlight/AI sources exist; mistake/heading sources, rich diagram preview, and backup-restore evidence remain |
| LEARN-003 | Daily review queue with Again/Hard/Good/Easy scheduling | `Partial` | Durable local-day limits, confidence, four grades, pause/bury/undo/crunch, live time, timezone/DST logic, and history analytics are covered; explicit overdue/learning semantics and calibrated history-based scheduling remain |
...
| LEARN-005 | Mistake notebook generated from failed reviews and assessments | `Backlog` | Acceptance tests not yet implemented |
...
| M7 | TEACH-001 | `Partial` | Automatic section slides, navigation, timer, recall concealment, narration, swipe/keyboard, and fullscreen | Authored ordered decks, speaker notes, audience-safe view, reusable templates, remote controls, AirPlay evidence, and PDF export |
...
Evidence gap: source behavior and main workflow paths are present, but focus-return,
body-scroll-lock, sidebar-action/route-close, repeated-toggle, and Mobile Safari
assertions are still required before `Verified`.
...
`src/lib/aiContract.js` defines `lumen.ai.request.v1`, `/api/health` and
...
Delivered slice: users can author basic, cloze, formula, derivation, compare,
...
explain, Socratic, quiz, flashcards, interview, summarize, and study-plan modes.
...
Answer-to-note insertion, code-review plus the remaining explicit prompt strategies,
directly navigable phone structured-field citations, personal-note-target navigation, claim-level
grounding enforcement, stronger unsupported-inference treatment, physical-iPhone
evidence, and versioned grounded-quality/interview evaluations remain.
...
material and is not a model-generated memory. Phone conversations are session-only. An
explicit saved no-AI preference, measured
latency/token/energy display, configurable Mac-history retention, app authentication
outside the single-learner trusted-LAN scope,
and trusted-HTTPS physical-iPhone model evidence remain open.
Deep is currently offered whenever the service is otherwise ready and sends
`think: true`; the reviewed Qwen model attests thinking support, but an operator-selected
completion-capable model that lacks it can fail. Capability-gating Deep (or formally
restricting supported models to thinking-capable ones) remains open.
...
Automated unit and browser fixtures pass, but grounded output is not provider-token-live,
phone structured-field citations are not inline links, and Mobile Safari, VoiceOver/live-
region, real model-generated diagram quality, interruption, and latency SLO evidence remain
open. The visible Approach is deterministic orchestration/evidence metadata; exposing raw
