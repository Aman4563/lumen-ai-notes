...
| 2026-09-24 | Tutor answer quality from the local model (issue #58, AI-001/AI-003) | Server-only fixes for problems found in real qwen3.5:4b runs. Socratic no longer receives a literal `Output pattern: Your question? [S1]` example, which the model had copied into 4 of 4 audit replies. It now gets a format description; after a tutor question it assesses the learner's answer in one or two sentences, then asks exactly one cited question. Fast prose now has an explicit target of about 150 words. Code review labels each finding as a Defect (traced to a failing input) or a Convention/alternative, avoids uncertain library-default claims, and warns against the one-pass variance shortcut. Grounded prose ends each source-backed paragraph with its label. Grouped or spaced labels the model wrote (`[S1, S2]`, `[S 1]`) are normalized; the server never adds a label to uncited text. A prose answer that arrives as bare JSON (even with valid citations) regenerates once as Markdown, then fails `AI_CONTRACT_ERROR`; source-free live prose holds back an opening `{` so it can be replaced. AI-003 nuance: an authorized search that returns no usable evidence no longer discards attached library passages. A Markdown request with library evidence gets a library-only answer that must cite `[S#]`, cannot contain `[W#]`, starts with a server-written "Current-web evidence unavailable" notice, and reports `webSearch.used: false`. `used` now means retained web evidence backs the answer. No library evidence, or a structured task, still fails closed with `WEB_SEARCH_NO_RESULTS`. The client still labels that state "not needed" and should map `requested && !used && rounds > 0` to its failed badge. Evidence: 42 new deterministic tests in `server/ai/quality.test.mjs`, covering both transports, the real browser client parsing the degraded envelope, and stream/envelope equality for every server-rewritten answer. `npm run check` passes, with `audit:ai` at 384/384 and `audit:ai-eval` passing. Live on this branch: the Socratic follow-up assessed then asked one `[S1]` question (14.0 s). Fast/Balanced L2 answers were 1,589/6,998 characters (390/1,642 tokens, 14.1/59.0 s), against audit Fast runs of 3,318 and 3,611 characters. 4 of 4 empty-search web fallbacks returned labeled library-only answers in 25.8–35.8 s, against a 23.5 s error before. Code review traced the off-by-one and labeled N vs N−1 a convention. 0 of 8 streamed grounded runs needed a citation regeneration. The filtered endpoint matrix passed 5/5 (details in docs/FUNCTIONAL_TESTING.md). Review follow-up: holding an opening `[` had stopped Markdown-link answers from streaming and lost their partial on a length stop, so only `{` is held; an indented library-only draft is trimmed so the notice cannot turn it into a code block. A Socratic follow-up whose history carried the old `Your question?` template assessed the answer and asked one `[S1]` question with no prefix (13.3 s), and a grounded follow-up after a library-only answer did not repeat the notice. |
| 2026-09-24 | Home and Library defect sweep (issue #52; SEARCH-001, PLAN-001, GRAPH-001, LEARN-004 partials) | Browser audits at 320–1280px against 36026b2 reproduced 29 Home/Library defects; each fix was re-measured on the new build. Search: an exclusion alone now excludes (`-regression` 143 → 91 results; a typographic minus or en dash folds to `-`), numeric `part:` compares the Part number (`part:5` 9 → 6 results, no Part 15), terms of four letters or fewer must start a word and all-caps acronyms must be whole words (`RAG` 100 capped → 14, the RAG chapter first, while `tran` still finds "transformer"), typo corrections score by field at 0.4× and the page names the corrected word (`regresion` now ranks the Regression chapters first), built-in lectures show case-preserved snippets cut only for returned results, and recent searches record committed queries only (Enter, blur, opening a result, or a 1.5 s pause) with stored prefixes pruned on load. Library: a one-row search bar that carries the focus ring, a short placeholder with visible syntax tips, `Part N` chips with `aria-pressed` and a scroll-edge fade (wrapping at ≥741px), a settled polite result count ("1 result", "Top 100 results"), and filter-specific empty states. Home: one `resumeTarget` rule (most recent unfinished lecture, else the next unread chapter) drives the hero, Continue card, Today tile, and plan; lectures opened by link or Back reach Recent; the fresh Start-reading tile opens the Library; the plan fills 15/30/60 minutes with up to four reading blocks and a reason each; the curriculum map is keyboard-operable buttons (one Tab stop, arrow keys) with an icon legend; readiness buttons name their Part and explain the 3-card minimum; the phone hero is 262px instead of a 440px band, and the phone study card (2,272 → 278px tall) and mastery rows no longer split words. Evidence: `npm run check` green (the unit audit adds exclusion, part, acronym, typo, snippet, and recent-search cases; `audit:scale` adds real-corpus semantics within the unchanged 2.5 s budget; plan tests cover the Continue rule and session lengths); `npm run check:browser` green with new assertions in `audit:workflow` (link-opened Recent, singular count, correction hint, a built-in `<mark>`, committed-only recents, `aria-pressed`), `audit:controls` (an enabled fresh tile, a one-stop map with arrow navigation), `audit:responsive` (no mid-word splits on Home or Library), and `audit:sync` (two idle tabs reading different lectures stop rewriting Recent; a first version of the link-open fix looped 1,337 revisions in 3 s); the link-open, fresh-tile, and word-split checks fail against the 36026b2 build. Theme-token contrast, the `<main>` landmark and skip link, and toast timing are tracked in #51. |
| 2026-09-24 | Accessibility foundation: theme contrast, focus, landmarks, and shell semantics (A11Y-001, BUG-003 partials) | Cross-cutting fixes from the 2026-09-24 UI audits. The Paper/Night `--ink-faint`, small coral/teal text, and white-on-coral primary buttons failed 2.4–3.3:1. They now use text-safe `--coral-text`/`--teal-text` and `--primary-bg`/`--on-primary` tokens. The undefined `--blue-soft`/`--danger`/`--accent`/`--sage` tokens are now defined in every theme, and `--ai-warn` follows the theme. The coral focus ring was 65% alpha, and the tutor rings were 26%. Focus now uses an opaque two-tone `--focus-ring` with a gold ring on navy surfaces, and the library search pill shows it. Buttons no longer show UA ButtonFace grey. Contrast flattens the hero, review hero, and token-driven sidebar to black. `overflow-wrap: anywhere` stacked flex labels one letter per line; it is now `break-word`, with learner-authored cards excepted. Forced colors keep button edges and mark the current item. The shell now has one `#main-content` main, a skip link, per-route titles, `aria-current`, and heading focus after in-app navigation. Toasts announce through persistent live regions with kind icons, and their dismiss timer no longer restarts on every render (HL-22). Every App dialog inerts the shell declaratively, and the closed drawer stays hidden after any dialog closes. The drawer is a modal dialog layered above the bottom navigation. Settings has a sticky close, headings, a theme radio group, and meters. The shell also shows an offline status pill, a visible shortcuts entry, and a Ctrl+K guard. Evidence: `src/lib/themeContrast.test.mjs` (15 tests: 4.5:1 text tokens, 3:1 focus ring, undefined-token guard) and the new `audit:a11y`. It covers 57 axe runs over 8 routes, Settings, and the drawer in Paper/Night/Contrast at phone and desktop, plus 144 structural checks, including keyboard scrolling after a click on lecture text and an undimmed focused curriculum Part. Its allowlist has three component-owned entries (concept map, whiteboard header, tutor connection notice). Both checks fail against the 2252c39 baseline. The responsive audit adds drawer paint-order and Settings close checks. Full `npm run check` (357 AI/data tests) + all thirteen browser audits green. VoiceOver and manual WCAG evidence remain open. |
| 2026-09-24 | AI tutor defects and accessibility (issue #56; AI-001/AI-004/A11Y-001) | A strict audit reproduced the defects against mocked and real `qwen3.5:4b` requests; each fix has a regression. Data: one tab no longer clones its own Library-first turn as a `sync-conflict-*` record or warns about a concurrent tab (saves read their merge base after the previous save commits; `audit:sync` still merges real concurrent tabs). Grounding: Library-first requests about the open lesson reserve up to six of its passages under the same byte fit and `[S#]` labels, and Choose sources lists the whole catalog with lazy lesson loading. Copy/export keep the Markdown source, render structured results as readable Markdown, list the sources behind `[S#]`/`[W#]` and mark incomplete answers. Lifecycle: Ask AI excerpts are consumed once, name the lecture, keep an unsent draft and focus the composer; drafts (tab-only sessionStorage) and the engine choice (per-browser localStorage) survive route changes; leaving mid-answer records a visible incomplete turn never sent as model memory; On-device Lite releases its model after a 45 s grace period or at once when hidden; flashcard duplicates report "Already in Review"; Refresh keeps an armed one-request web permission. Rendering: structured fields render sanitized inline KaTeX; answer headings start at h4; wide tables and equations are named focusable scroll regions; the hidden copy status no longer extends the page; chips, Depth and actions wrap as units. Accessibility: focus never falls to `<body>` after Generate, completion, Stop, Check answer, Save, Clear or pairing; one polite region announces phases and outcomes; the grounding radiogroup has one Tab stop with arrow keys; Clear uses an in-app dialog; a learner's Stop is a neutral note; setup states show one card; tutor text, badges and CTA clear 4.5:1 in Paper, Night and Contrast (axe colour-contrast clean for the tutor). Evidence: `npm run check` (349 AI/data tests, AI eval 27 cases hit@1 0.913); `audit:ai-ui` adds single-tab history, open-lesson grounding, catalog, lifecycle and keyboard/screen-reader scenarios; `audit:responsive` 407 layout and 342 control checks with a seeded tutor turn; all 12 browser suites pass; live `audit:ai-live-ui` 8/8 modes against the real model. Landmarks, the global focus ring, Reader code-block scrolling and Review-deck math remain with their owning issues. |
| 2026-09-24 | AI tutor review follow-up (issue #56) | An adversarial review of the branch reproduced six more defects, each confirmed against the preceding build and fixed with a regression. A learner could not scroll back inside a streaming answer (following ignored every scroll within 150 ms of its own); On-device Lite's Ask AI box slid below the fold while its device panel loaded; general questions such as "the current state of the art" reserved open-lesson passages; a keyboard Stop landing as retrieval finished could leave a stale phase in the status region (1 of 100 probed stops); an approved web search that kept no evidence (the #64 library-only answer) showed "Web fallback not needed" beside the server's notice and now shows the failed-fallback badge; the Depth select and source filter were 42px on phones. Evidence: `npm run check` (349 AI/data tests, AI eval 27 cases hit@1 0.913); `audit:ai-ui` adds scroll-back, Ask AI reveal on both engines against the fixed bars, a delayed Stop re-read, the degraded-web badge and a 44px check of every Mac tutor control on a 393px phone; all 12 browser suites pass. Live `qwen3.5:4b`: an unedited Library-first Explain from the Linear Regression lesson attached 6 of 8 passages from that lesson and cited them (47 s); a follow-up and a three-question quiz (13 KaTeX renders, no raw `$`) left exactly three user and three answer turns, no `sync-conflict-*` record and no warning; Copy gave readable quiz Markdown. |
| 2026-09-25 | AI tutor on the accessibility foundation (issue #56; A11Y-001) | The tutor branch was rebased onto main after the Reader, Whiteboard, answer-quality, Library/Home, startup-bundle and accessibility-foundation merges. The rewritten Mac tutor keeps the foundation's `<section aria-label="Conversation">` instead of a nested `<main>`. Its focus rings, the confirmation dialog and the On-device quiz feedback use the shared 3px `--focus-ring` with its `--focus-halo` band; the source filter shows the ring on its label. Success, error and warning text now alias the theme's `--teal-text`, `--danger` and `--ai-warn` instead of tutor-local mixes. The Review center and its dialogs stay on demand (startup entry 713,112 bytes, budget 750,000), and the profile save queue fix is unchanged. The `.ai-tutor__connection` colour-contrast exemption is removed from `scripts/a11y_audit.mjs`. Evidence: `npm run check` (424 AI/data tests, AI eval 27 cases hit@1 0.913); all 13 browser suites pass on the first run, including `audit:ai-ui` (single-tab history), `audit:sync`, `audit:responsive` (434 layout and 367 control checks) and `audit:a11y` (57 axe runs, 0 allowlisted nodes). |
| 2026-09-24 | Offline route screens and in-shell chunk failures (BUG-004, PERF-001, DATA-002 partials; issue #59) | With the app server actually stopped, a fresh install that had opened only Home replaced the whole app with "Lumen needs fresh app files" on Read, AI Tutor, and Settings, and reload repeated it: the service worker cached only the entry files, and every update discarded route chunks again. The build now emits `offline-routes.json` (Reader, Whiteboard, AI studio, both tutors, Storage health, Device evidence, with their static imports and CSS), and install precaches it: 19 files and 688,530 bytes beyond the 908 KB entry, sent uncompressed by the Node server (about 196 KB from a gzip host); KaTeX's JavaScript accounts for 259 KB because the tutor imports it. Lectures, search data, Mermaid, fonts, and the WebLLM runtime stay on demand. Install time (median of five fresh profiles, loaded Mac): 143 → 612 ms on localhost, 253 → 738 ms at 50 Mbps/10 ms, 366 → 1,509 ms at 10 Mbps/60 ms, and 957 → 4,776 ms at 1.6 Mbps/150 ms; registration still waits for the load event. Install rejects a list from another build or with another entry, fetches every file before writing, fails on a missing or HTML-answered file, and deletes only its own unused cache; a worker registered without a build query (legacy or the dev server, which `audit:phone-ai-ui` exercises) installs entry-only. Scratch partial releases (missing Reader chunk; previous build's list) left the prior release active and cached. "Remove optional offline files" also deleted the route chunks; it now keeps every file in the current and each installed list. While an update waited, the worker's cache-first handler overwrote the active cache's list with the new build's, and a second cleanup removed the active release's route screens and entry script; the worker no longer answers or caches that list, and two cleanups over a waiting update now leave the active cache complete. A chunk that still fails renders inside the shell with Home and reload actions, resets on navigation, and distinguishes offline, unreachable, and incomplete builds. Chunk recovery probes `/api/health` before its automatic reload. Storage health has its own boundary so backup export survives, and an undownloadable lecture gets a plain-language message. Evidence: `audit:visual` now stops its own servers instead of calling `setOfflineMode` (which does not block service-worker fetches). After a Home-only visit it opens Read, both AI engines, Whiteboard, Review, Device evidence, and Settings, evaluates every route screen's module from the cache, and reloads a visited lecture; against the foundation build it reports the fatal screen. Its main pass runs the optional cleanup and asserts that the lecture is gone and every route file remains. `audit:chunks` adds offline, missing-file, and Storage-health cases; `audit:app` checks the route list's contents, entry, and 900 KB bound; `chunkRecovery.test.mjs` adds 4 probe/recovery tests (361 AI/data tests). Full `npm run check` and all thirteen browser audits green. Physical iPhone install/offline evidence remains open. |
| 2026-09-24 | Offline route screens: review follow-up (BUG-004, DATA-002 partials; issue #59) | An adversarial review found the server-stopped `audit:visual` checks still passing with a mutated worker that never answered from Cache Storage. Chrome's HTTP cache, filled by the worker's install fetches of `immutable` assets, answered them after the server stopped, and the visited-lecture step navigated to the URL it was already on, a same-document fragment change that never reloaded. The audit now clears the HTTP cache after each stop, asserts that the server no longer answers, and reloads the lecture; the mutated worker now fails every screen and the reload. Repair app files deleted the caches and then called `update()`, which never reinstalls an unchanged worker URL. The route screens stayed uncached until the next release (AI Tutor failed with the server stopped), and a waiting worker whose cache Repair had just deleted could be promoted. Repair now unregisters the worker; the reload reinstalls the build with all 22 listed files, and `audit:visual` runs that sequence. On Home itself the in-shell panel's primary Go to Home was a no-op; Reload is primary there now. Evidence: `chunkRecovery.test.mjs` asserts that Repair unregisters and neither updates nor promotes a worker (361 AI/data tests); full `npm run check` and all thirteen browser audits green. Physical iPhone evidence remains open. |
| 2026-09-25 | Offline route screens after the startup-bundle split and accessibility foundation (BUG-004, PERF-001, A11Y-001 partials; issue #59) | Rebased onto #66 and #61, `audit:visual` failed with the server stopped: after a Home-only visit, Review did not render (`Failed to fetch dynamically imported module: …/ReviewCenter-….js`), and the module check found the review center, its card editor, the readiness check, and the reader's lazy TeX renderer (`markdownMath.js`) missing from `offline-routes.json`. The list now names them: 34 files, 30 of them (773,930 bytes, about 223 KB gzipped) beyond the 714,320-byte entry, which stays under the 750 KB startup budget. Because the split moved code from the entry into route files, the entry plus route files fell from about 1.60 MB to 1.49 MB. The card editor and readiness check render outside the view container, so a chunk that still fails there reaches the app-level boundary. The in-shell boundary sits inside `<main id="main-content">`: an ad-hoc check at 393 and 1280 px found one main landmark and skip link, `main` inert and `aria-hidden` under Settings and restored after, and zero axe violations on the panel in Paper, Night, and Contrast. A first failure keeps focus on the main landmark and announces itself through the panel's alert; returning to the failed screen focuses the panel's `h1`. Evidence: `audit:app` and `audit:visual` name all four modules; `audit:chunks` asserts the single landmark and return-visit focus and fails when route focus skips the panel heading. `npm run check` (421 AI/data tests) and all thirteen browser audits passed on the first attempt. The `audit:a11y` allowlist is unchanged: its three entries belong to the concept map, whiteboard header, and tutor notice. Physical iPhone evidence remains open. |
| 2026-09-24 | Review center, readiness check, and notebook defects (issue #54; CONTENT-002, LEARN-003/005, ASSESS-002, PLAN-002, A11Y-001 partials) | Browser testing reproduced a crash and a set of usability defects; each fix carries a regression. Card import: ReviewCenter never received `onImportCards` (the parameter sat on InterviewRound since 421918d), so every `lumen.cards.v1` import threw a ReferenceError and Wave 7's validated import had never worked; Import is now a keyboard-operable button and reports imported, duplicate, over-capacity, and malformed-file outcomes. Sessions: the grading panel sticks above the phone bottom navigation (all four grades were hidden at 375×667 and 320×640), the progress bar is a `progressbar` reporting graded/total (it showed 1/remaining), focus moves to each prompt, the revealed answer, and back to Start, and grades are announced in a polite status. Due counts: Home's widget and Review button, the sidebar badge, and the PLAN-002 app badge share `actionableReviewCount` (today's queue: no suspended, archived, or buried-today cards, within limits); Home had shown three counts for one deck, and crunch practice now ends with its session instead of leaving the hero counting its practice pool. The hero separates an empty deck from a clear queue and explains locked actions visibly; the forecast buckets by calendar day with Today/weekday labels and text zero-states instead of skeleton dashes; the FSRS Daily limits strip wraps (it collapsed to 0–11px at 768–1100px) and keeps whole one-word labels on phones. Dialogs: the mistake dialog renders outside the view and makes the shell inert, restoring each region's prior state; a readiness check in progress asks before a scrim tap, Escape, or close discards answers, focuses each question and the result, uses a roving-tabindex radio group, and submits cloze answers on Enter. Notebook: mistake and clipping deletions offer a focused Undo in the deleted entry's place, scrolled into view, that reinserts the same record and never expires while it holds focus; clipping notes buffer keystrokes and, like mistake corrections, commit before the page hides (burst typing had dropped a key with React error #185); small-phone rows keep two-line titles; the tile reads Personal notes; link reports name documents by title; review and notebook fields are 16px on phones (iOS focus zoom). Evidence: `audit:review` grew keyboard deck import with duplicate and malformed-file feedback, Home due-count agreement, session focus/progress/announcement and 375×667 grade reach, the phone Daily limits strip in both schedulers, the inert mistake dialog with drawer-state restore, mistake and clipping Undo in the deleted entry's slot and on screen (including a hovered, focused strip that must not expire), the hero after crunch practice, archive focus handoff, an all-cloze readiness check submitted with Enter, burst-typed clipping notes, and a correction saved on page hide without blur; `audit:workflow` grew the readiness leave guard, radio arrow keys, result focus, and title-named link reports; unit coverage for the shared due count (buried, archived, limits), calendar-day forecast buckets, over-capacity import counts, and record reinsertion. Rebased onto the accessibility foundation (#61), which shipped the items this row had left to #51 (toasts over sheets, theme-token contrast, App-level dialog inert handling with the drawer restore, the Notebook sidebar badge): the mistake dialog now joins the App modal flag instead of setting `inert` itself, so a dialog opened and closed over it no longer re-exposes the shell (`audit:review` asserts this and fails against the previous mechanism), and an axe pass over populated review screens fixed the mistake chip and Show answer hint contrast and the missing practice-view headings. Open and owned elsewhere: the `<form role="dialog">` markup shared by the review and App dialogs (axe `aria-allowed-role`); Home's per-Part readiness labels (#52); Review in the phone bottom nav stays a product decision. |
| 2026-09-25 | Tutor answers cannot forge citation controls (issue #69; AI-001/AI-004) | The tutor rendered model output through marked, which passes raw HTML through, and DOMPurify's default profile, which keeps `<button>` and `data-*`. Against the preceding build, a mocked answer containing `<button class="ai-tutor__citation" data-ai-citation="S2">` rendered a real button, and clicking it opened the S2 lesson; a forged `<span>` and `<a>` also carried `data-ai-citation`. No `[S#]` marker produced that control, so the citation validator never saw it (§18: untrusted content cannot declare its own citation labels). The tutor's marked instance no longer tokenizes raw HTML: block and inline tags become escaped text in prose and structured fields on both engines, and a bare `<br>` is the only model HTML kept. `[S#]` buttons and `[W#]` links are rendered by a marked extension from markers outside code, with each answer's evidence passed as a parse option; a `[S#]:` line stays a visible citation instead of a hidden link definition. Tutor link labels are parsed Markdown, and link titles and code-fence languages are escaped: a quote in either had added attributes, `data-ai-citation` and `style` included. The Reader's renderer is unchanged, and 14 legitimate answers (the audit and fixture answers among them) render byte-identical markup before and after. Evidence: `audit:ai-ui` mocks an answer with a forged button, span, anchor and `onerror` image and expects them as text, one citation control, a plain paragraph under the forged label and no navigation after a click there; against the preceding build it fails with four `data-ai-citation` elements. `audit:phone-ai-ui` checks the same for a forged button in the phone answer. Unit tests run on the renderer output before DOMPurify (which needs a DOM) and cover forged buttons, `data-ai-*` on other elements, scripts, frames, event handlers, forms, link labels and titles, fence info strings, raw-text tags and structured fields, plus citations, math, code, tables and Mermaid source. Open: AI answers saved to notes and AI flashcards added to the deck render later through the Reader and Review renderer, which keeps author HTML for DOMPurify and has the same link-title and fence-language quoting; those screens have no citation handler. |
| 2026-09-25 | Tutor citation forgery: review follow-up (issue #69; AI-001/AI-004) | An adversarial review found raw HTML, entities, SVG/MathML, forms, autolinks, link titles, image alt text, table/list/blockquote HTML, KaTeX `\href`/`\htmlData` and Mermaid labels, directives and theme CSS inert, but three paths still reached a destination no evidence supplied. A model could wrap a verified citation in its own link, `[[S1]](#/read/notes/forged-phone-route)`: Chrome follows an `<a>` when a `<button>` inside it is clicked, and the phone handler did not prevent that, so the S1 source opened and the hash then moved to the model's route (on the Mac, a plain click was prevented, but the chip still sat inside the model's link). A Markdown link to an app route, `[Open the lecture](#/read/notes/…)`, opened a note no citation validated. The Mermaid SVG filter kept any `#` href, so a `click` line to `#/read/…` would become a working link once Mermaid declares `xlink` (11.17 does not, so such diagrams fail to parse today). Tutor link labels that show a citation marker (entities, full-width forms and zero-width characters included) now render without the link, tutor links keep only `http(s)`/`mailto` targets, image alt text shows the marker instead of button markup, the phone citation handler prevents the default action, and diagram links lose their targets while `<use>` references stay; no lecture uses a Mermaid `click` line. Evidence: `audit:phone-ai-ui` clicks the wrapped `[S1]` and fails against the previous build when the hash moves; `audit:ai-ui` expects two renderer `[S#]` buttons, no links and no navigation from the route label; `audit:mermaid` renders a well-formed linked SVG through a stand-in Mermaid and fails when a link target survives; unit tests fail when either link check or the image override is removed. Open: remote Markdown images in tutor answers still load (`img-src https:`), a tracking and exfiltration path outside #69. |
| 2026-09-25 | Saved AI output renders as untrusted text; tutor images never load (issue #81; AI-001/AI-004/LEARN-002) | The #69 review named three paths outside the tutor's rules. Against main, a seeded AI flashcard rendered in Review through the Reader's renderer: a model `<button class="ai-tutor__citation" data-ai-citation="S1">` became a real button, a raw `<img>` and a Markdown image to `https://tracker.example/…` rendered and the deck sent 16 requests to that host, and a link to the app's own origin was live. "Create review card" on an AI clipping dropped its provenance. In tutor answers a Markdown image loaded (`img-src https:`), `[x](http://<app-host>/#/read/…)` was a live link, and `[x](http:#/read/…)` passed the http(s) check although the browser resolves it against the page to an app route. The tutor's untrusted rules now live in `untrustedMarkdown.js` (no KaTeX): a remote Markdown image becomes a link that loads nothing, other images show their alt text, links to the app's host render as text, and kept links carry the normalized address. Review renders cards tagged `ai-draft` or made from an `ai-tutor` clipping with those rules in the deck, sessions, interview rounds and the dialog preview; saved `[S#]`/`[W#]` markers stay text. A card made from an AI clipping gets `ai-draft`, and an edit cannot drop it. Learner cards and the Reader are unchanged; no path opens saved AI text in the Reader, and the Notebook already shows it as plain text. No schema field was added. Evidence: unit tests cover images, same-host links, saved-text structure, the Reader renderer and provenance, and each of nine mutations (image override, host check, normalization, URL parse, citation-label check, nested images, html tokenizers, clipping clause, tag order) fails at least one; `audit:ai-ui` saves a hostile answer and hostile flashcards and checks the tutor, the Notebook clipping, the review dialog preview and the deck; `audit:review` seeds AI and pre-tag cards and checks the deck, a session, an interview round, edit previews and tag persistence; both fail against main and assert no request reaches the image host. KaTeX `\includegraphics` renders nothing. Mermaid diagrams did fetch; see the review follow-up row. Open: the Reader still loads images in learner content. |
| 2026-09-25 | Saved AI output: review follow-up (issue #81; AI-004/LEARN-002) | An adversarial review found the Markdown rules held against protocol tricks (`HTTP:`, `//`, `http:\\`, `http:host`, entity-encoded schemes, tabs, user info, numeric IPv4 forms, ports), `<picture>`/`srcset`, CSS `url()` in `style` and KaTeX `\href`/`\htmlStyle`/`\color{url(…)}`. Two defects remained. (1) Mermaid diagrams in tutor answers fetched remote resources while Mermaid drew them, before the SVG filter ran. In Chrome, tracker.example received requests from a `fontFamily` directive, `themeCSS` with selectors, a frontmatter `config:`, an `htmlLabels` directive with an `<img>` label, a flowchart node's `@{ img }`, a sequence actor's `properties` icon, and a state-diagram `classDef … url()`. The earlier note that Chrome made no request was wrong. (2) A card made from an AI clipping before #81 lost its provenance through a card export, a logged mistake or the clipping's deletion. The untrusted profile now marks model diagrams, and Mermaid renders them with every config key in `secure`, so no directive or frontmatter config applies. A model diagram whose source names a web scheme, `//`, a backslash, `url(`, `image-set` or `@import`, after numeric entities are decoded, is shown as code. Learner diagrams keep their directives. The profile normalizer adds `ai-draft` to cards made from AI clippings. Evidence: `audit:mermaid` asserts no remote or same-origin label request, code blocks for address-naming model diagrams, no applied model directive and a still-applied learner directive. Against the previous renderer it fails with 11 tracker requests, and with only the config lock removed it fails on the label request. Unit tests cover 13 address spellings, the locked config and the renderer marker, and 10 mutations each fail a test. The provenance test covers load, clipping deletion, export and import, and a corrective card, and `audit:review` checks the deck tag before any edit. `npm run check` (459 AI/data tests; startup entry 713,759 bytes) and all 13 browser suites passed on the first attempt. Open: Safari/WebKit was not run; a `lumen.cards.v1` file from someone else is treated like an uploaded note. |
| 2026-09-24 | AI tutor depth: phone-first composer and study features (issue #57; AI-001, AI-004, LEARN-005, INTERVIEW-001, A11Y-001 partials) | Built on the #56 fixes without changing the request contract, the server or the profile shape: every feature is a visible question through one shared fit path (exact UTF-8 measurement, the profile's byte budget, whole `[S#]` blocks and turns only) or local UI state. Layout: the composer docks above the bottom navigation with an auto-growing box, Send/Stop and an Options sheet for depth, answer length, web fallback and privacy; the engine picker is one line and phones pick the mode from a native select; Enter/Shift+Enter/Up/Esc work on keyboards only; streaming stops following when the learner scrolls back, with Jump to latest; grounded answers show staged progress announced once per step. Study: suggested starts from the learner's lessons, mistakes and lapsed cards; one-tap follow-ups that remember only the answer they follow; quiz confidence, a once-announced score, a Fast answer check without score or strengths, saving misses to the mistake notebook and a weak-spot quiz; Listen through the app's speech engine; New topic (replacing Clear, with export) and a three-hour context break. Practice: a Socratic/Interview session strip (question count, Hint, I'm stuck, Wrap up over the session's own turns, no grading); rubric-graded practice of the 76 authored interview questions through a hidden `answer_feedback` mode whose one supplied source is the model answer and rubric, blocked when that reference would be clipped, with the rubric as a checklist, no score, the reference answer on request and misses filed like timed-round misses; Work through with tutor from the mistake notebook and readiness-check results (consumed once, never sent, draft-protecting, hidden with AI off); and Library-first requests name their first attached lesson as the topic. Evidence: `npm run check` (505 AI/data tests; AI eval 27 cases, hit@1 0.913; startup entry 713,637 bytes of 750,000; route screens 897,679 bytes of 900,000, with interview practice loading with its bank); all 13 browser suites pass (`npm run check:browser`: `audit:ai-ui` 107 s, `audit:responsive` 434 layout and 367 control checks, `audit:a11y` 57 axe runs; workflow and review passed on retry while another worktree's browser gate shared the machine, then first time in a separate run). New unit tests cover the session derivation and wrap-up window, the practice reference and weak-first order, the bridge prompts, the topic cue and the whole-reference check; `audit:ai-ui` checks the dock at five viewports (now with a session strip) and each new flow's payload, focus and 44px targets, and `audit:phone-ai-ui` a prepared question on On-device Lite. Live `qwen3.5:4b` on a 393px phone: Socratic turns and a hint in 10–12 s with resolved citations; the first Wrap up failed its grounding check on `[S#]` labels copied from session memory (labels are now dropped; the rerun took 21 s); two authored answers graded in 17 s and 20 s, schema-valid, their gaps matching exactly the rubric points each answer missed (hidden scores 35 and 10); a bridged mistake answered in 13 s but retrieved off-topic passages until it searched with its own question and expected answer. Socratic prompt issues seen live (praise for a "previous answer" on the first turn, explaining before asking) are left to the server work in #58. |
| 2026-09-24 | AI tutor depth: review follow-up (issue #57; AI-001, LEARN-005, INTERVIEW-001, A11Y-001) | An adversarial review confirmed ten defects against the preceding build or live and fixed each with a regression: a follow-up of a follow-up searched with the chip's wording (live, a ridge answer's Check my understanding retrieved the problem-framing chapter), so follow-ups now search with the learner's question behind the chain on both engines; a recap with no learner answer credited the hint's points; an answer from before the three-hour break offered follow-ups that would send it as memory; Edit & reuse, regenerate and Up arrow on a graded practice answer put its grading question in the box without the rubric (they now reopen the practice card); prepared questions from the mistake notebook and readiness checks overflowed On-device Lite's 1,800-character box; "?" opened the shortcut sheet under Request options and the New topic dialog; at 200% text on a 320px phone the dock covered the whole tutor (it now stays in the page flow past 60% of the room above the navigation); Enter and a refused send gave no reason; a failed reading gave no reason; and the quiz dispute note denied a saved miss. Live `qwen3.5:4b` (8 generations) confirmed the chained search and the recap. After rebasing onto #83 the quiz views load with the first quiz to keep route screens under budget (903,321 → 890,629 bytes). Gate: `npm run check` (524 AI/data tests; AI eval hit@1 0.913; startup entry 715,643 bytes) and all 13 browser suites on the first attempt. |
| 2026-09-25 | Tutor session turns framed by the server; Checking citations shown (issue #82; AI-001, AI-004) | Live `qwen3.5:4b` runs on main praised answers the learner never gave. Examples: "Your previous assessment correctly identified…" on a session start, "Your hint correctly identifies…" on the Hint action, the same on Check my understanding and on Next question. A mistake worked through from the notebook opened by correcting the old answer instead of asking what went wrong. The Socratic instruction's condition ("when the learner has just answered…"), sent on every turn, is gone. The server now chooses one framing per turn: `diagnose` for a notebook mistake, `hint` for the Hint action, `open` for a message that says it has not answered or that follows no tutor question, and `answer` when the tutor's last turn ends by asking a question (an opening offer or "Does that make sense?" does not count), after a hint, or for a `My answer:` reply. The request contract, eval fixtures and envelope did not change. The client's action prompts say "I have not answered…" and the bridge says to ask first and explain only after the reply. Wording saved in older conversations maps to the same framing. The rule reads history as the client window sends it (whitespace collapsed, labels kept). Streaming: `validating` came only after a grounded draft had passed, never for a failed draft or on the JSON transport. It also came in the same read as the answer, so the Checking citations step never drew. Both transports now report `validating` before each terminal draft is checked, bounded by the one-shot recoveries. The tutor waits 300 ms on it while its steps are visible (not on a hidden page; Stop ends the wait). The stream client stops instead of applying buffered text after a Stop during that wait, and each step is announced once per answer. Evidence: 25 framing cases built through the client's own builders and window and checked again after server validation, plus phase-order tests on both transports and the real stream endpoint (`server/ai/quality.test.mjs`), client prompt, hold and Stop tests, and an `audit:ai-ui` scenario. That scenario sends the integrated server's own stream to the page in one piece; Checking citations must be drawn for at least 2 frames from the `validating` event and announced once. Removing the hold fails it with "Checking citations was never on screen". Live on this branch: session starts, Check my understanding and Next question asked one cited question with no praise (4 of 4 starts). 6 of 6 hints gave a hint and asked to try again with no assessment, though 4 of 6 stated most of the mechanism. 6 of 6 notebook mistakes opened with one diagnostic question only (27–33 tokens). The reply to it and answers after a question or a hint were assessed, then followed by one question. Grounded answers streamed `generating > validating > delta > complete`. In headless Chrome at 393 px, Checking citations was drawn for 18 frames (283 ms) and announced once in 2 of 2 answers; on main it was drawn in 0 and 1 frames and never announced. Details in docs/FUNCTIONAL_TESTING.md. |
| 2026-09-25 | Tutor session framing: review follow-up (issue #82; AI-001, AI-004) | An adversarial review chained live `qwen3.5:4b` sessions through the real client and this worktree's integrated server, feeding each reply back as history (73 generations). It confirmed the Hint action, session starts, Check my understanding, Next question, reveals, wrap-ups and the notebook bridge as submitted, and the request contract, eval fixtures and phase order unchanged. Four defects remained. (1) "Give me a hint." typed after a question was framed as an answer: "Your answer is **partly correct**…". Short typed hint requests are now framed `hint` (2 of 2 live: a hint, no assessment), and a short "Next question, please." or "Skip this one" is framed `open`; both are matched only in the learner's own first paragraph and up to 120 characters, so a longer answer that mentions a hint is still assessed. (2) Text written above a notebook mistake in the question box hid its opening words, and its own `My answer:` line made it an answer to correct ("Your answer is a misconception because…", 89 tokens). The bridge is now also recognized by its `Question:` and `Expected answer:` lines (2 of 2 live: one diagnostic question, 27–36 tokens). (3) 3 of 20 live assessed turns ended with a task ("…, consider how…") instead of a question, so the learner's reply was not assessed. A closing task now counts as asking, as do `? [S1].` and `？`, and the answer and open formats ask the model to end with the question. (4) The earlier start and Check my understanding wording (a stale shell or restored draft) was framed as an answer after a real question; it is now `open`. The `open` instruction no longer claims the learner has answered nothing, which is false mid-session. Remaining: after a reveal in a long session, Next question still credited earlier answers in about 1 of 4 runs with either wording (framing `open` every time); session starts and Check my understanding 0 of 8 each. Evidence: 16 new framing cases through the client's window in `server/ai/quality.test.mjs`, each rule mutation-checked (2 failing tests each; the old open wording fails 1); details in docs/FUNCTIONAL_TESTING.md. Gate: `npm run check` (`audit:ai` 542/542, AI eval 27 cases with hit@1 0.913 and no fixture change; startup entry 715,643 bytes; route screens 891,828 bytes) and all 13 browser suites on the first attempt. |
...
| M5 | AI-004 | `Partial` | Strict NDJSON ordering/terminal checks, source-free token delivery, grounded/schema buffering, Stop/cancellation, bounded Markdown display, DOMPurify GFM/KaTeX with model-authored HTML shown as text (no forged citation controls, no image loads, no links to the app; saved AI flashcards and clippings keep those rules in Review), deferred sanitized Mermaid (model diagrams with locked config and no resource loads), theme rerender, and malformed-diagram diagnostics pass automated unit/browser audits | Define response-latency SLOs; verify VoiceOver/live-region behavior and Mobile Safari rendering/cancellation; add a real-host update/interruption matrix; decide and document whether any provisional grounded draft may ever be displayed |
...
or route-screen assets are missing or the route list belongs to another build.
Recoverable lazy imports recognize missing fingerprinted JS and CSS, probe the server,
perform at most one online automatic reload per tab/cooldown, and otherwise show an
in-shell message (offline, unreachable, or incomplete build). An incomplete build offers
an online-verified manual repair that removes only Lumen application-file caches. IndexedDB,
...
Issue #52 corrected the slice: an exclusion alone excludes, numeric `part:`
values match exactly, short terms and all-caps acronyms match on word
boundaries, typo corrections rank by field and are named in the results,
built-in lectures show case-preserved match snippets, and recent searches
record only committed queries.
...
Part nodes are keyboard-operable buttons (one Tab stop, arrow keys along the
path) with a state legend that pairs each color with an icon. The
...
quiz/flashcard/study-plan string fields render navigable [S#]/[W#] citations and
sanitized inline KaTeX math on both the Mac and phone surfaces, and a personal-note
citation opens the Reader's Notes drawer and focuses the exact note editor. Library-first
requests about the open lesson reserve its passages; Copy and export keep the Markdown
source with a per-answer source list. Explicit derivation/analogy/challenge
